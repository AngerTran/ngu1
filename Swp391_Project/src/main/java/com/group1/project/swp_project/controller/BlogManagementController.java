...
public class BlogManagementController {
    ...
    @Operation(summary = "Tạo một bài viết mới", description = "Tạo một bài viết mới với trạng thái 'Chờ duyệt'. Yêu cầu vai trò STAFF.")
    @PostMapping
    ...
    @Operation(summary = "Cập nhật một bài viết đã có", description = "Cập nhật tiêu đề, nội dung, hoặc hình ảnh của một bài viết. Yêu cầu vai trò ADMIN, MANAGER, hoặc STAFF.")
    @PutMapping("/{id}")
    @PreAuthorize("hasAnyAuthority('Admin', 'Manager', 'Staff')")
    public ResponseEntity<Blog> updateBlog(@PathVariable int id, @RequestBody @Valid CreateBlogRequest request) {
        // Cần thêm logic kiểm tra xem người sửa có phải là người tạo bài không, hoặc là
        // admin/manager
        return ResponseEntity.ok(blogService.updateBlog(id, request));
    }

    // API xóa bài viết: Cả 3 vai trò đều được xóa
    @Operation(summary = "Xóa một bài viết", description = "Xóa vĩnh viễn một bài viết. Yêu cầu vai trò ADMIN, MANAGER, hoặc STAFF.")
    @DeleteMapping("/{id}")
    @PreAuthorize("hasAnyAuthority('Admin', 'Manager', 'Staff')")
    ...
    @Operation(summary = "Lấy danh sách bài viết chờ duyệt", description = "Trả về danh sách các bài viết có trạng thái 'Chờ duyệt'. Yêu cầu vai trò ADMIN hoặc MANAGER.")
    @GetMapping("/pending")
    ...
    @Operation(summary = "Phê duyệt một bài viết", description = "Thay đổi trạng thái của bài viết từ 'Chờ duyệt' sang 'Đã đăng'. Yêu cầu vai trò MANAGER.")
    ...
    @Operation(summary = "Từ chối một bài viết", description = "Thay đổi trạng thái của bài viết từ 'Chờ duyệt' sang 'Bị từ chối'. Yêu cầu vai trò MANAGER.")
    ...
    @Operation(summary = "Lấy tất cả bài viết", description = "Trả về danh sách tất cả các bài viết, không phân biệt trạng thái. Yêu cầu vai trò ADMIN, MANAGER hoặc STAFF.")
    @GetMapping("/all")
    @PreAuthorize("hasAuthority('Manager')")
    ...
    @Operation(summary = "Xem chi tiết một bài viết bất kỳ", description = "Xem chi tiết bài viết theo ID, không giới hạn trạng thái. Yêu cầu vai trò ADMIN, MANAGER hoặc STAFF.")
    @GetMapping("/{id}")
    @PreAuthorize("hasAnyAuthority('Admin', 'Manager', 'Staff')")
